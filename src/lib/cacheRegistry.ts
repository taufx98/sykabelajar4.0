import { getPersistentCache, removePersistentCache, clearPersistentCache } from '@/lib/persistentCache';
...
export function clearUserCache(userId: string, resources?: string[]): void {
  const names = resources ?? ['notifications', 'awards', 'certificates', 'orders', 'chat.unread', 'profile'];
  for (const resource of names) removePersistentCache(userCacheKey(resource, userId));
}

export function clearPublicCache(resources?: Array<'home' | 'stats' | 'leaderboard' | 'coinLeaderboard' | 'competitions' | 'feed' | 'profiles'>): void {
  const names = resources ?? ['home', 'stats', 'leaderboard', 'coinLeaderboard', 'competitions', 'feed', 'profiles'];
  for (const name of names) {
    ...
    if (name === 'profiles') clearPersistentCache('public.profile.');
    try {
...
export function invalidateForRealtime(resource: string, userId?: string): void {
  switch (resource) {
    ...
    case 'profile': clearPublicCache(['home', 'leaderboard', 'coinLeaderboard', 'profiles']); if (userId) clearUserCache(userId, ['profile']); return;
