...
export function clearPublicCache(resources?: Array<'home' | 'stats' | 'leaderboard' | 'coinLeaderboard' | 'competitions' | 'feed'>): void {
  ...
  for (const name of names) {
    ...
    try {
      ...
      if (name === 'leaderboard') {
        for (const limit of [10, 20, 50, 100]) localStorage.removeItem(`${LEGACY_PUBLIC_PREFIX}leaderboard.${limit}`);
      }
      if (name === 'coinLeaderboard') {
        for (const limit of [10, 20, 50, 100]) localStorage.removeItem(`${LEGACY_PUBLIC_PREFIX}coinLeaderboard.${limit}`);
      }
    } catch {
...
export function hasCachedValue<T>(key: string): boolean {
  return getPersistentCache<T>(key) !== null;
}

export function invalidateForRealtime(resource: string, userId?: string): void {
  switch (resource) {
    case 'competition':
      clearPublicCache(['home', 'stats', 'competitions']);
      return;
    case 'leaderboard':
      clearPublicCache(['home', 'leaderboard', 'coinLeaderboard']);
      return;
    case 'feed':
      clearPublicCache(['home', 'feed']);
      return;
    case 'banner':
    ...
    case 'profile':
      clearPublicCache(['home', 'leaderboard', 'coinLeaderboard']);
      if (userId) clearUserCache(userId, ['profile']);
      return;
    case 'notification':
      if (userId) clearUserCache(userId, ['notifications']);
      return;
    case 'order':
      if (userId) clearUserCache(userId, ['orders']);
      return;
    case 'award':
      if (userId) clearUserCache(userId, ['awards', 'certificates']);
      return;
    default:
      return;
  }
