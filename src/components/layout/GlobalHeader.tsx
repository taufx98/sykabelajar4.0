...
function resolveAdminHeader(location: ReturnType<typeof useLocation>): HeaderMeta {
  if (location.pathname === '/admin/error-intelligence') {
    return { section: 'Monitoring', title: 'Error Intelligence', subtitle: 'Pemantauan sistem dan incident', icon: ShieldAlert };
  }
  if (location.pathname === '/admin/plan-usage') {
    return { section: 'Admin', title: 'Plan & Usage', subtitle: 'Penggunaan paket dan kapasitas', icon: SlidersHorizontal };
  }
  if (location.pathname === '/admin/organizers') {
    return { section: 'Admin', title: 'Organisasi', subtitle: 'Manajemen penyelenggara', icon: Building2 };
  }
  if (location.pathname === '/admin/chat') {
    return { section: 'Komunikasi', title: 'Chat Admin', subtitle: 'Percakapan dan tiket pengguna', icon: ShieldAlert };
  }
  if (location.pathname === '/admin/core') {
    const tab = new URLSearchParams(location.search).get('tab');
    const tabs: Record<string, { title: string; icon: HeaderMeta['icon'] }> = {
      competitions: { title: 'Lomba', icon: Trophy },
      users: { title: 'Pengguna', icon: Users },
      posts: { title: 'Postingan', icon: FileText },
      orders: { title: 'Pesanan', icon: ShoppingBag },
      shop: { title: 'Shop', icon: Store },
      settings: { title: 'Pengaturan', icon: Settings },
    };
...
export function GlobalHeader() {
  ...
  const meta = resolveAdminHeader(location);
