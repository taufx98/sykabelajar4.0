...
function ProfileMessagingGate() {
  ...
  useEffect(()=>{
    ...
    const findHost=()=>setHost(document.querySelector<HTMLElement>('[data-syka-profile-actions-host]'));
    findHost();
  ...
  useEffect(()=>{
    let alive=true;setProfile(null);setStatus('none');
  ...
  const handleFollow=async()=>{if(busy)return;setBusy(true);try{const result=await requestFollow(profile.id);const next=(result.status as FollowStatus)||'pending';setStatus(next);toast(next==='approved'||next==='auto'?'Sekarang kamu mengikuti pengguna ini.':'Permintaan mengikuti terkirim.','success');}catch(e:any){toast(e?.message??'Gagal mengikuti pengguna.','error');}finally{setBusy(false);}};
  const remove=async()=>{if(busy)return;setBusy(true);try{const {error}=await supabase.rpc('remove_follow',{p_target_user_id:profile.id});if(error)throw error;setStatus('none');toast('Berhenti mengikuti pengguna.','info');}catch(e:any){toast(e?.message??'Gagal membatalkan follow.','error');}finally{setBusy(false);}};
  return createPortal(<div className="flex justify-center gap-2" aria-live="polite">{status==='approved'||status==='auto'?<><Button size="sm" variant="outline" onClick={()=>void remove()} disabled={busy} icon={<UserMinus size={14}/>}>Unfollow</Button><Link to={`/admin/chat?user_id=${profile.id}`}><Button size="sm" variant="primary" icon={<MessageCircle size={14}/>}>Kirim Pesan</Button></Link></>:status==='pending'?<Button size="sm" variant="outline" disabled icon={<Clock3 size={14}/>}>Diminta</Button>:<Button size="sm" variant="primary" onClick={()=>void handleFollow()} disabled={busy} icon={<UserPlus size={14}/>}>Ikuti</Button>}</div>,host);
}
