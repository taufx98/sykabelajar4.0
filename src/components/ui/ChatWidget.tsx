        ...
        <div className="mb-3 w-80 h-[420px] surface-card-bg border surface-border rounded-2xl shadow-2xl overflow-hidden flex flex-col animate-slide-up">
            ...
            <div className="flex items-center gap-2.5">
              ...
              <div>
                ...
                <p className="text-[10px] text-moss-200">
                  {view === 'chat' ? 'Online' : view === 'waiting' ? 'Menunggu balasan...' : view === 'ended' ? 'Chat selesai' : 'Siap membantu'}
                </p>
              </div>
            </div>
          ...
          {view === 'form' && (
            <div className="flex-1 overflow-y-auto p-4 space-y-4">
              <div className="text-center mb-2">
                <div className="w-14 h-14 rounded-full bg-moss-500/10 flex items-center justify-center mx-auto mb-3"><MessageCircle size={24} className="text-accent" /></div>
              ...
              <div>
                ...
                <input type="text" value={title} onChange={e => setTitle(e.target.value)} placeholder="Contoh: Masalah Login" className="input" />
              </div>
              <div>
                <label className="text-xs text-slate-400 font-medium mb-1.5 block">Deskripsi Masalah</label>
                <textarea value={description} onChange={e => setDescription(e.target.value)} placeholder="Jelaskan masalah kamu secara detail..." rows={4} className="input resize-none" />
              </div>
              <button onClick={handleSubmitForm} disabled={!title.trim() || !description.trim() || loading} className="w-full py-2.5 rounded-xl bg-moss-500 hover:bg-moss-600 text-sm font-medium text-white disabled:opacity-40 transition flex items-center justify-center gap-2">
          ...
          {view === 'waiting' && (
            <div className="flex-1 flex flex-col items-center justify-center p-6 text-center">
              <div className="w-16 h-16 rounded-full bg-amber-500/10 flex items-center justify-center mx-auto mb-4"><Headphones size={28} className="text-amber-400" /></div>
              ...
              <button onClick={handleStartChat} className="px-4 py-2 rounded-xl bg-moss-500 hover:bg-moss-600 text-sm font-medium text-white transition">Lihat Chat</button>
            </div>
          )}
          ...
          {view === 'chat' && (
            <>
              <div className="flex-1 overflow-y-auto p-3 space-y-3 surface-elevated min-h-0">
                {loading && <div className="text-center py-4"><div className="w-5 h-5 border-2 border-moss-400 border-t-transparent rounded-full animate-spin mx-auto" /></div>}
                {!loading && messages.length === 0 && <div className="text-center py-8"><MessageCircle size={24} className="mx-auto text-slate-600 mb-2" /><p className="text-xs text-slate-500">Belum ada pesan</p></div>}
                {messages.map(msg => {
                  const isMe = msg.sender_id === user?.id;
                  return (
                    <div key={msg.id} className={`flex gap-2 ${isMe ? 'justify-end' : 'justify-start'}`}>
                      {!isMe && <div className="w-7 h-7 rounded-full bg-moss-500/20 flex items-center justify-center shrink-0 mt-1"><Headphones size={12} className="text-accent" /></div>}
                      <div className="max-w-[75%]">
                        <p className={`text-[10px] mb-0.5 ${isMe ? 'text-right text-slate-500' : 'text-slate-500'}`}>{isMe ? (user.displayName || 'Kamu') : 'Admin'}</p>
                        <div className={`px-3 py-2 rounded-2xl text-sm ${isMe ? 'bg-moss-600 text-white rounded-br-sm' : 'surface-elevated text-fg-secondary rounded-bl-sm'}`}>
                          <p className="whitespace-pre-wrap">{msg.body}</p>
                          <p className={`text-[9px] mt-1 ${isMe ? 'text-moss-200' : 'text-slate-500'}`}>{new Date(msg.created_at).toLocaleTimeString('id-ID', { hour: '2-digit', minute: '2-digit' })}</p>
                        </div>
                      </div>
                      {isMe && <Avatar name={user.displayName} id={user.id} size={28} src={user.profilePhoto || undefined} />}
                    </div>
                  );
                })}
                <div ref={messagesEnd} />
                ...
                {thread?.status === 'closed' ? (
                  <div className="text-center space-y-2">
                    ...
                    <div className="flex gap-2 justify-center">
                      <button onClick={() => setView('rating')} className="px-3 py-1.5 rounded-lg bg-amber-500/10 text-amber-400 text-xs font-medium hover:bg-amber-500/20 transition">⭐ Beri Rating</button>
                      <button onClick={handleNewChat} className="px-3 py-1.5 rounded-lg bg-moss-500/10 text-accent text-xs font-medium hover:bg-moss-500/20 transition flex items-center gap-1"><Plus size={12} /> Chat Baru</button>
                    </div>
                  </div>
                ) : (
          ...
          {view === 'ended' && (
            <div className="flex-1 flex flex-col items-center justify-center p-6 text-center">
              <div className="w-16 h-16 rounded-full bg-slate-500/10 flex items-center justify-center mx-auto mb-4"><MessageCircle size={28} className="text-slate-400" /></div>
              ...
              <button onClick={handleNewChat} className="px-5 py-2.5 rounded-xl bg-moss-500 hover:bg-moss-600 text-sm font-medium text-white transition flex items-center gap-2"><Plus size={16} />Mulai Chat Baru</button>
            </div>
          )}
          ...
          {view === 'rating' && (
            <div className="flex-1 flex flex-col items-center justify-center p-6 text-center">
              ...
              <div className="flex gap-1 mb-4">
                {[1, 2, 3, 4, 5].map(s => <button key={s} onClick={() => setRating(s)} className="transition active:scale-110"><Star size={32} className={s <= rating ? 'text-amber-400 fill-amber-400' : 'text-slate-600'} /></button>)}
              </div>
              <div className="flex gap-2 w-full">
                <button onClick={handleSkipRating} className="flex-1 py-2.5 rounded-xl surface-elevated text-slate-400 text-sm font-medium hover:surface-elevated transition">Lewati</button>
                <button onClick={handleRate} disabled={rating === 0} className="flex-1 py-2.5 rounded-xl bg-moss-500 hover:bg-moss-600 text-sm font-medium text-white disabled:opacity-40 transition">Kirim</button>
              </div>
            </div>
          )}
        </div>
