...
export async function getHomeSnapshot(feedLimit = 15): Promise<HomeSnapshot> {
  ...
  const now = Date.now();
  if (homeMemory && homeMemory.expiresAt > now && homeMemory.data.feed.length >= Math.min(feedLimit, 15)) return homeMemory.data;
  ...
  homeInFlight = (async () => {
    ...
    const result: HomeSnapshot = {
      ...
      leaderboard: Array.isArray(payload.leaderboard) ? (payload.leaderboard as Array<Record<string, unknown>>).map((row) => ({
        ...
        institution: row.institution == null ? null : String(row.institution), avatar_url: row.avatar_url == null ? null : String(row.avatar_url),
        grade: row.grade == null ? null : String(row.grade), xp: Number(row.xp ?? 0), rank: Number(row.rank ?? 0),
        rank_change: row.rank_change === 'up' || row.rank_change === 'down' ? row.rank_change : 'same', point_change: Number(row.point_change ?? 0),
      })) : [],
      coin_leaderboard: Array.isArray(payload.coin_leaderboard) ? (payload.coin_leaderboard as Array<Record<string, unknown>>).map((row) => ({
        user_id: String(row.user_id), username: String(row.username ?? ''), display_name: String(row.display_name ?? row.username ?? 'Pengguna'),
        institution: row.institution == null ? null : String(row.institution), avatar_url: row.avatar_url == null ? null : String(row.avatar_url),
        grade: row.grade == null ? null : String(row.grade), edu_coin: Number(row.edu_coin ?? 0), rank: Number(row.rank ?? 0),
      })) : [],
      stats: (() => {
        const row = (payload.stats ?? {}) as Record<string, unknown>;
        return { total_users: Number(row.total_users ?? 0), total_students: Number(row.total_students ?? 0), total_schools: Number(row.total_schools ?? 0), total_competitions: Number(row.total_competitions ?? 0), total_public_competitions: Number(row.total_public_competitions ?? 0), total_certificates: Number(row.total_certificates ?? 0) };
      })(),
  ...
  try { return await homeInFlight; } catch { return emptyHomeSnapshot(); } finally { homeInFlight = null; }
}
...
export async function getPlatformStats(): Promise<PlatformStats> {
  ...
  if (cached) { statsMemory = { expiresAt: now + CACHE_TTL.stats, data: cached }; return cached; }
  if (statsFailureUntil > now) return emptyStats();
  if (statsInFlight) return statsInFlight;

  statsInFlight = (async () => {
    const { data, error } = await supabase.rpc('get_platform_stats');
    if (error) { statsFailureUntil = Date.now() + NEGATIVE_CACHE_TTL; throw error; }
    const row = Array.isArray(data) ? data[0] : data;
    const result = { total_users: Number(row?.total_users ?? 0), total_students: Number(row?.total_students ?? 0), total_schools: Number(row?.total_schools ?? 0), total_competitions: Number(row?.total_competitions ?? 0), total_public_competitions: Number(row?.total_public_competitions ?? 0), total_certificates: Number(row?.total_certificates ?? 0) };
    statsMemory = { expiresAt: Date.now() + CACHE_TTL.stats, data: result };
    statsFailureUntil = 0;
    writeCache('stats', result, CACHE_TTL.stats);
    return result;
  })();
  try { return await statsInFlight; } catch { return emptyStats(); } finally { statsInFlight = null; }
}
...
export async function getPublicLeaderboard(limit = 100): Promise<PublicLeaderboardRow[]> {
  ...
  if (cached) { leaderboardMemory.set(safeLimit, { expiresAt: now + CACHE_TTL.leaderboard, data: cached }); return cached; }
  ...
  try { return await leaderboardInFlight; } catch { return []; } finally { leaderboardInFlight = null; }
}
...
export async function getPublicCoinLeaderboard(limit = 100): Promise<PublicCoinLeaderboardRow[]> {
  ...
  if (cached) { coinLeaderboardMemory.set(safeLimit, { expiresAt: now + CACHE_TTL.coinLeaderboard, data: cached }); return cached; }
  ...
  try { return await coinLeaderboardInFlight; } catch { return []; } finally { coinLeaderboardInFlight = null; }
}
...
export async function getPublicCompetitions(limit = 6): Promise<Array<Record<string, unknown>>> {
  ...
  if (cached) { competitionsMemory = { expiresAt: now + CACHE_TTL.competitions, data: cached }; return cached.slice(0, safeLimit); }
  ...
  try { const rows = await competitionsInFlight; return rows.slice(0, safeLimit); } catch { return []; } finally { competitionsInFlight = null; }
}
