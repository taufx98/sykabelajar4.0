...
import { GuruCollectivePage } from '@/pages/GuruCollectivePage';
import { CollectiveParticipantLoginPage } from '@/pages/CollectiveParticipantLoginPage';
import { CollectiveParticipantPortalPage } from '@/pages/CollectiveParticipantPortalPage';
...
function RoleRoute({ role, children }: { role: 'admin' | 'organizer_member' | 'teacher'; children: ReactNode }) { const { isAuthenticated, authLoading, user } = useApp(); const allowed = !!user && (role === 'admin' ? user.role === 'admin' : role === 'teacher' ? user.role === 'guru' || user.role === 'admin' : user.role === 'penyelenggara' || user.role === 'admin'); if (authLoading) return <div className="min-h-screen flex items-center justify-center surface-bg"><p className="text-xs text-fg-muted">Memuat sesi...</p></div>; if (!isAuthenticated) return <Navigate to="/login" replace />; if (!allowed) return <Navigate to="/home" replace />; return <>{children}</>; }
