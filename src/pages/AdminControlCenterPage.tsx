...
export function AdminControlCenterPage() {
  const groups = useMemo(() => ({
    operasi: MODULES.slice(0, 6),
    platform: MODULES.slice(6, 10),
    akses: MODULES.slice(10),
  }), []);
  ...
  return <div className="min-h-screen surface-bg text-fg-secondary">
    <header className="sticky top-0 z-30 glass border-b surface-border">
      <div className="max-w-7xl mx-auto px-4 md:px-6 py-4 flex items-center justify-between">
        <div className="flex items-center gap-3 min-w-0">
          ...
          <div className="min-w-0"><p className="text-[10px] text-accent font-semibold uppercase tracking-[0.16em]">SYKABELAJAR</p><h1 className="font-display text-xl font-bold text-fg truncate">Panel Admin</h1></div>
        </div>
    ...
    <main className="max-w-7xl mx-auto p-4 md:p-6 space-y-6">
      <section><p className="text-xs text-accent font-semibold uppercase tracking-wide">Control Center</p><h2 className="font-display text-2xl md:text-3xl font-bold text-fg mt-1">Pusat Kendali Platform</h2><p className="text-sm text-fg-muted mt-2 max-w-3xl">Semua modul administrasi terhubung melalui alur yang jelas. Gunakan modul spesifik untuk pekerjaan operasional tanpa mengubah design system SykaBelajar.</p></section>
      <section><AdminDashboard /></section>

      {(['operasi','platform','akses'] as const).map(group => <section key={group}>
        <div className="flex items-center gap-2 mb-3"><h3 className="text-sm font-semibold text-fg">{group === 'operasi' ? 'Operasional Utama' : group === 'platform' ? 'Platform & Konten' : 'Akses, Paket & Keamanan'}</h3><span className="h-px flex-1 bg-white/5" /></div>
        <div className="grid sm:grid-cols-2 xl:grid-cols-3 gap-3">
          {groups[group].map(({ title, description, path, icon: Icon, badge }) => <Link key={title} to={path} className="group">
            <Card hover className="h-full p-4 md:p-5 transition-colors">
              <div className="flex items-start gap-3"><div className="w-11 h-11 rounded-xl bg-moss-500/10 flex items-center justify-center shrink-0 group-hover:bg-moss-500/15"><Icon size={19} className="text-accent" /></div><div className="min-w-0 flex-1"><div className="flex items-center gap-2"><h4 className="text-sm font-semibold text-fg group-hover:text-accent truncate">{title}</h4>{badge && <Badge color="info">{badge}</Badge>}</div><p className="text-[11px] leading-relaxed text-fg-muted mt-1.5">{description}</p></div><span className="text-fg-muted group-hover:text-accent text-lg">→</span></div>
            </Card>
          </Link>)}
        </div>
      </section>)}

      <section className="rounded-xl border border-amber-500/15 bg-amber-500/5 p-4"><p className="text-xs font-semibold text-amber-300">Aturan Admin</p><p className="text-[11px] text-fg-muted mt-1">Aksi sensitif harus divalidasi backend/RLS. UI hanya menjadi antarmuka; keberhasilan ditentukan respons Supabase.</p></section>
    </main>
  </div>;
}
