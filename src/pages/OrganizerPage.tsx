...
const safeOrganizerFields = 'id,name,slug,description,logo_asset_url,owner_user_id,status,created_at,updated_at';
  ...
  const load = async () => {
    ...
    if (!org) {
      ...
      if (membership) {
        ...
        org = memberOrg ? { ...memberOrg, _memberRole: membership.role || membership.member_role || 'editor' } as any : null;
      }
    }
            ...
            <div className="space-y-3">
              <button onClick={() => setShowCreateForm(true)} className="w-full p-4 rounded-xl surface-card-bg border surface-border hover:border-moss-500/30 hover:surface-elevated transition-all duration-200 text-left group active:scale-[0.98]">
                <div className="flex items-center gap-3">
                  <div className="w-10 h-10 rounded-xl bg-moss-500/10 flex items-center justify-center group-hover:bg-moss-500/15 transition"><Plus size={18} className="text-accent" /></div>
                  <div className="flex-1"><p className="text-sm font-semibold text-fg group-hover:text-accent transition">Buat Organisasi Baru</p><p className="text-[11px] text-slate-500">Daftarkan organisasi kamu</p></div>
  ...
  return (
    <div className="min-h-screen flex surface-bg">
      ...
      <section className="flex-1 p-5 md:p-7 overflow-auto">
        <div className="md:hidden flex items-center gap-3 mb-4"><Link to="/home" className="p-2 rounded-lg hover:bg-surface-elevated/50 text-slate-400"><ArrowLeft size={18} /></Link><h1 className="font-bold text-fg truncate">{organizer.name}</h1></div>
...
function Editor({ title, children, onClose, onSave, busy }: { title: string; children: any; onClose: () => void; onSave: () => void; busy: boolean }) {
  return <div className="fixed inset-0 z-50 bg-black/70 backdrop-blur-sm flex items-center justify-center p-4 animate-in fade-in"><div className="w-full max-w-xl surface-card-bg border surface-border rounded-2xl shadow-2xl p-5 animate-in slide-in-from-bottom-4"><div className="flex justify-between items-center mb-4"><h3 className="font-bold text-fg">{title}</h3><button onClick={onClose} className="text-slate-400 hover:text-fg transition"><X size={18} /></button></div><div className="space-y-3">{children}</div><div className="flex justify-end gap-2 mt-5 pt-4 border-t surface-border"><Button variant="outline" onClick={onClose}>Batal</Button><Button onClick={onSave} loading={busy}>Simpan</Button></div></div></div>;
}
