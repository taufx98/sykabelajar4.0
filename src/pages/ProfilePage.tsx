...
import { Award, Calendar, Filter, GraduationCap, MapPin, MessageCircle, School, Settings2, Trophy, UserMinus, Users, X } from 'lucide-react';
...
function ProfileBadgeCollection({ badges, own }: { badges: ProfileBadge[]; own: boolean }) {
  ...
  return <div>
    <div className="flex flex-col gap-3 border-b surface-border pb-4 sm:flex-row sm:items-end sm:justify-between">
      <div>
        <div className="flex items-center gap-2"><div className="flex h-9 w-9 items-center justify-center rounded-xl bg-amber-500/10 text-amber-500"><Trophy size={18}/></div><div><p className="font-display text-lg font-bold text-fg">Koleksi Badge</p><p className="text-xs text-fg-muted mt-0.5">{badges.length} badge telah diperoleh. Terus aktif dan raih lebih banyak pencapaian!</p></div></div>
      </div>
      <div className="flex flex-wrap gap-2">
        <select className="input pr-8 text-xs" value={category} onChange={(e) => setCategory(e.target.value)} aria-label="Filter kategori">{categories.map((value) => <option key={value}>{value === 'Semua' ? 'Semua Kategori' : value}</option>)}</select>
        <select className="input pr-8 text-xs" value={rarity} onChange={(e) => setRarity(e.target.value)} aria-label="Filter rarity">{rarities.map((value) => <option key={value}>{value === 'Semua' ? 'Semua Rarity' : value}</option>)}</select>
        <select className="input pr-8 text-xs" value={sort} onChange={(e) => setSort(e.target.value)} aria-label="Urutan badge"><option>Terbaru</option><option>Nama</option></select>
      </div>
    </div>
...
export function ProfilePage() {
    ...
    (async () => {
      ...
      try {
        ...
        const [a, s, u, b, v, registrations] = await Promise.all([
          ...
          supabase.from('registrations').select('id', { count: 'exact', head: true }).eq('user_id', p.data.id).in('status', ['APPROVED', 'ACTIVE']).then((result) => result),
        ]);
  ...
  return <div className="min-h-screen bg-slate-50/40 pb-12 dark:bg-slate-950/20">
    <div className="px-3 sm:px-5 md:px-8 pt-4">
      <div className="mx-auto max-w-6xl overflow-hidden rounded-2xl border surface-border surface-card-bg shadow-sm">
    ...
    <div className="px-3 sm:px-5 md:px-8">
      <div className="mx-auto mt-4 max-w-6xl overflow-x-auto border-b surface-border">
        <div className="flex min-w-max justify-start md:justify-center">{tabs.map(([value, label]) => <button key={value} type="button" onClick={() => setTab(value)} className={`border-b-2 px-4 py-3 text-sm font-medium whitespace-nowrap transition ${tab === value ? 'border-accent text-accent' : 'border-transparent text-fg-muted hover:text-fg'}`}>{label}</button>)}</div>
      </div>
