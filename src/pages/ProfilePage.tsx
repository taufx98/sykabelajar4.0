...
function ProfileBadgeCollection({ badges, own }: { badges: ProfileBadge[]; own: boolean }) {
  ...
  return <div>
    <div className="flex flex-col gap-3 border-b surface-border pb-4 sm:flex-row sm:items-center sm:justify-between">
      ...
      <div className="grid grid-cols-3 gap-2 sm:flex"><select className="input w-[150px] pr-8 text-xs" value={category} onChange={(e) => setCategory(e.target.value)} aria-label="Filter kategori">{categories.map((value) => <option key={value} value={value}>{value === 'Semua' ? 'Semua Kategori' : value}</option>)}</select><select className="input w-[135px] pr-8 text-xs" value={rarity} onChange={(e) => setRarity(e.target.value)} aria-label="Filter rarity">{rarities.map((value) => <option key={value} value={value}>{value === 'Semua' ? 'Semua Rarity' : value}</option>)}</select><select className="input w-[110px] pr-8 text-xs" value={sort} onChange={(e) => setSort(e.target.value)} aria-label="Urutan badge"><option>Terbaru</option><option>Nama</option></select></div>
    </div>
  ...
  return <div className="min-h-screen bg-slate-50/40 pb-12 dark:bg-slate-950/20">
    <div className="px-3 pt-4 sm:px-5 md:px-8">
      <div className="mx-auto max-w-6xl overflow-hidden rounded-3xl border surface-border surface-card-bg shadow-sm">
        <div className="relative h-36 overflow-hidden sm:h-40 md:h-44 surface-bg">
          ...
          <div className="absolute inset-x-0 bottom-0 flex items-end justify-between gap-4 p-4 sm:p-5 md:p-6">
            <p className="max-w-xl text-sm font-medium text-white/90 sm:text-base">“Belajar hari ini, untuk masa depan yang lebih besar.”</p>
            {own && <Link to="/profile/edit" className="hidden sm:inline-flex"><span className="inline-flex items-center gap-1.5 rounded-xl border border-white/20 bg-slate-950/45 px-3 py-2 text-xs font-semibold text-white backdrop-blur hover:bg-slate-950/65"><Camera size={14}/> Edit Cover</span></Link>}
          </div>
        </div>
