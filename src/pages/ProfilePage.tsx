...
import { Award, Calendar, Camera, Filter, MessageCircle, School, Settings2, Trophy, UserMinus, Users, X } from 'lucide-react';
...
const ACCOUNT_TYPE_LABELS: Record<string, string> = {
  admin: 'Admin', administrator: 'Admin', student: 'Pelajar', peserta: 'Pelajar', teacher: 'Guru', guru: 'Guru', organizer: 'Penyelenggara', penyelenggara: 'Penyelenggara',
};
const PROFILE_FIELDS = 'id,username,full_name,verification_type,grade,subjects,bio,institution,birth_date,avatar_url,cover_url,is_public,badge_showcase,total_xp,edu_coin,account_type';
...
function ProfileBadgeCollection({ badges, own }: { badges: ProfileBadge[]; own: boolean }) {
  ...
  const filtered = badges.filter((badge) => (category === 'Semua' || badge.category === category) && (rarity === 'Semua' || badge.rarity === rarity)).sort((a, b) => sort === 'Terbaru' ? new Date(b.awarded_at).getTime() - new Date(a.awarded_at).getTime() : a.name.localeCompare(b.name));
  ...
  return <div>
    <div className="flex flex-col gap-3 border-b surface-border pb-4 sm:flex-row sm:items-center sm:justify-between">
      ...
      <div className="grid grid-cols-3 gap-2 sm:flex"><select className="input w-[160px] pr-8 text-xs" value={category} onChange={(e) => setCategory(e.target.value)} aria-label="Filter kategori">{categories.map((value) => <option key={value} value={value}>{value === 'Semua' ? 'Semua Kategori' : value}</option>)}</select><select className="input w-[145px] pr-8 text-xs" value={rarity} onChange={(e) => setRarity(e.target.value)} aria-label="Filter rarity">{rarities.map((value) => <option key={value} value={value}>{value === 'Semua' ? 'Semua Rarity' : value}</option>)}</select><select className="input w-[110px] pr-8 text-xs" value={sort} onChange={(e) => setSort(e.target.value)} aria-label="Urutan badge"><option>Terbaru</option><option>Nama</option></select></div>
    </div>
    <div className="mt-4 grid grid-cols-3 gap-3 sm:grid-cols-4 md:grid-cols-5 lg:grid-cols-6 xl:grid-cols-8">{filtered.map((badge) => <BadgeCard key={badge.id} badge={badge}/>)}</div>
...
export function ProfilePage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        if (alive) {
          setProfile(p.data); setAwards(a.error ? [] : (a.data || [])); setFollowers(Number(sr?.follower_count || 0)); setFollowing(Number(sr?.following_count || 0)); setCompetitionCount(registrations.error ? 0 : (registrations.count ?? 0)); setShowSocial(ui?.show_social_popup !== false); setShowFollowing(ui?.show_following_popup !== false); setBadges(b); setBadgeVisibility(v);
        }
        if (user && user.id !== p.data.id) { const fs = await getFollowStatus(user.id, p.data.id).catch(() => 'none' as FollowStatus); if (alive) setFollow(fs); }
      } catch (e: any) { if (alive) { setProfile(null); toast(e?.message || 'Profil gagal dimuat.', 'error'); } }
  ...
  useEffect(() => {
    ...
    let alive = true;
    (async () => { try { const r = await supabase.rpc('get_profile_connections', { p_profile_id: profile.id, p_mode: social }); if (r.error) throw r.error; if (alive) setRows(r.data || []); } catch (e: any) { if (alive) toast(e?.message || 'Daftar pengguna gagal dimuat.', 'error'); } })();
    return () => { alive = false; };
  ...
  const accountTypeLabel = ACCOUNT_TYPE_LABELS[accountType] || (profile?.account_type ? String(profile.account_type) : '');
  ...
  const doFollow = async () => {
    ...
    const removing = follow === 'approved' || follow === 'auto'; const old = follow; const next: FollowStatus = removing ? 'none' : profile.is_public ? 'approved' : 'pending';
    setBusy(true); setFollow(next); if (removing) setFollowers((v) => Math.max(0, v - 1)); else if (next === 'approved') setFollowers((v) => v + 1);
  ...
  return <div className="min-h-screen bg-slate-50/40 pb-12 dark:bg-slate-950/20">
    <div className="px-3 pt-4 sm:px-5 md:px-8">
      <div className="mx-auto max-w-6xl overflow-hidden rounded-3xl border surface-border surface-card-bg shadow-sm">
        <div className="relative h-44 overflow-hidden sm:h-52 md:h-56 surface-bg">
          ...
          {own && <Link to="/profile/edit" aria-label="Edit cover" className="absolute right-4 top-4 inline-flex items-center gap-1.5 rounded-xl border border-white/20 bg-slate-950/55 px-3 py-2 text-xs font-semibold text-white shadow-sm backdrop-blur transition hover:bg-slate-950/75"><Camera size={14}/> Edit Cover</Link>}
        </div>
        <div className="relative px-4 pb-5 sm:px-6 md:px-7">
          <div className="-mt-12 flex flex-col gap-4 md:-mt-16">
            <div className="flex flex-col gap-4 md:flex-row md:items-end">
              <div className="relative flex shrink-0 justify-center md:justify-start"><ProfilePortrait name={profile.full_name || profile.username} id={profile.id} src={profile.avatar_url || undefined}/>{own && <Link to="/profile/edit" aria-label="Edit foto profil" className="absolute bottom-1 right-1 flex h-8 w-8 items-center justify-center rounded-full border-2 border-white bg-slate-900 text-white shadow-md dark:border-slate-950"><Camera size={14}/></Link>}</div>
              <div className="min-w-0 flex-1 pb-1 text-center md:text-left">
                <div className="flex flex-wrap items-center justify-center gap-2 md:justify-start"><h1 className="font-display text-2xl font-bold tracking-tight text-fg md:text-3xl">{profile.full_name || profile.username}</h1><VerifiedMark type={profile.verification_type}/></div>
                <p className="mt-0.5 text-sm font-medium text-fg-muted">@{profile.username}</p>
                <div className="mt-2 flex flex-wrap items-center justify-center gap-2 md:justify-start">{accountTypeLabel && <span className="rounded-full bg-accent/10 px-2.5 py-1 text-[11px] font-semibold text-accent">{accountTypeLabel}</span>}{grade && <span className="rounded-full bg-slate-500/10 px-2.5 py-1 text-[11px] font-semibold text-fg-muted">{grade}</span>}</div>
                <div className="mt-3 rounded-xl surface-elevated px-3 py-2.5 text-left"><p className="text-[10px] font-semibold uppercase tracking-[0.16em] text-fg-muted">Bio</p><p className="mt-1 text-sm leading-relaxed text-fg">{profile.bio || 'Belum ada bio.'}</p></div>
                <div className="mt-2 flex flex-wrap justify-center gap-x-4 gap-y-1 text-xs text-fg-muted md:justify-start">{profile.institution && <span className="flex items-center gap-1"><School size={13}/>{profile.institution}</span>}{profile.birth_date && <span className="flex items-center gap-1"><Calendar size={13}/>Lahir {new Date(profile.birth_date).toLocaleDateString('id-ID', { day: 'numeric', month: 'short', year: 'numeric' })}</span>}</div>
              </div>
              <div className="flex shrink-0 justify-center gap-2 pb-1 md:justify-end">{own ? <><Link to="/profile/edit"><Button size="sm" variant="primary">Edit Profil</Button></Link><Link to="/profile/interface-settings"><Button size="sm" variant="outline" icon={<Settings2 size={14}/>}>Tampilan & Privasi</Button></Link></> : user && <><Button size="sm" disabled={busy} variant={follow === 'approved' || follow === 'auto' ? 'outline' : 'primary'} onClick={() => void doFollow()}>{follow === 'pending' ? 'Diminta' : follow === 'approved' || follow === 'auto' ? 'Berhenti Mengikuti' : 'Ikuti'}</Button>{(follow === 'approved' || follow === 'auto') && <Link to={`/pesan?user_id=${profile.id}`}><Button size="sm" variant="outline" icon={<MessageCircle size={14}/>}>Kirim Pesan</Button></Link>}</>}</div>
            </div>
          </div>
          <div className="mt-5 grid grid-cols-3 gap-2 border-y surface-border py-3 sm:flex sm:items-center sm:justify-end sm:gap-8"><button type="button" disabled={!showFollowing} onClick={() => showFollowing && setSocial('following')} className="group min-w-[72px] rounded-xl px-3 py-1.5 text-center transition hover:bg-accent/5 disabled:opacity-50"><b className="block text-lg text-fg">{following}</b><span className="text-xs text-fg-muted group-hover:text-fg">Mengikuti</span></button><button type="button" disabled={!showSocial} onClick={() => showSocial && setSocial('followers')} className="group min-w-[72px] rounded-xl px-3 py-1.5 text-center transition hover:bg-accent/5 disabled:opacity-50"><b className="block text-lg text-fg">{followers}</b><span className="text-xs text-fg-muted group-hover:text-fg">Pengikut</span></button><div className="min-w-[92px] rounded-xl px-3 py-1.5 text-center"><b className="block text-lg text-fg">{competitionCount}</b><span className="text-xs text-fg-muted">Lomba Diikuti</span></div></div>
          {canShowBadges && resolvedShowcase.length > 0 && <div className="mt-3 flex flex-wrap items-center justify-center gap-2 md:justify-start">{resolvedShowcase.map((badge) => <div key={badge.id} className="inline-flex items-center gap-2 rounded-full border surface-border surface-elevated px-2.5 py-1.5"><div className="flex h-7 w-7 items-center justify-center">{badge.icon_url ? <img src={optimizedCloudinaryUrl(badge.icon_url, { width: 64 })} alt="" className="h-7 w-7 object-contain"/> : <span className="text-lg" role="img" aria-label={badge.name}>{badge.icon_emoji || '🏅'}</span>}</div><div className="min-w-0"><p className="max-w-[150px] truncate text-xs font-semibold text-fg">{badge.name}</p><p className="text-[10px] text-fg-muted">{badge.rarity}</p></div></div>)}</div>}
        </div>
      </div>
    </div>
    <div className="px-3 sm:px-5 md:px-8"><div className="mx-auto mt-4 max-w-6xl overflow-x-auto border-b surface-border"><div className="flex min-w-max justify-start gap-1 md:justify-center">{tabs.map(([value, label]) => <button key={value} type="button" onClick={() => setTab(value)} className={`border-b-2 px-4 py-3 text-sm font-medium whitespace-nowrap transition ${tab === value ? 'rounded-t-xl border-accent bg-accent/5 text-accent' : 'border-transparent text-fg-muted hover:bg-slate-500/5 hover:text-fg'}`}>{label}</button>)}</div></div>
      <Card className="mx-auto mt-4 max-w-6xl p-4 sm:p-5 md:p-6">{tab === 'tentang' && <div className="grid gap-3 md:grid-cols-2"><div className="rounded-2xl surface-elevated p-4"><p className="text-xs font-medium text-fg-muted">Bio</p><p className="mt-1 text-sm leading-relaxed text-fg">{profile.bio || 'Belum ada bio.'}</p></div><div className="rounded-2xl surface-elevated p-4"><p className="text-xs font-medium text-fg-muted">Pendidikan</p><p className="mt-1 text-sm text-fg">{profile.institution || 'Belum diisi'}</p><p className="mt-1 text-xs text-fg-muted">{grade || 'Jenjang belum diisi'}</p></div><div className="rounded-2xl surface-elevated p-4"><p className="text-xs font-medium text-fg-muted">Identitas Akun</p><p className="mt-1 text-sm text-fg">{accountTypeLabel || 'Belum diisi'}</p><p className="mt-1 text-xs text-fg-muted">{profile.username ? `@${profile.username}` : 'Username belum diisi'}</p></div></div>}{tab === 'prestasi' && <div className="space-y-2">{awards.slice(0, 10).map((a) => <div key={a.id} className="flex items-center gap-3 rounded-xl p-2"><Award size={18} className="text-accent"/><div><p className="font-semibold text-fg">{a.title}</p><p className="text-xs text-fg-muted">{a.subtitle || a.rank_code || 'Penghargaan'}</p></div></div>)}{!awards.length && <p className="text-sm text-fg-muted">Belum ada prestasi.</p>}</div>}{tab === 'lomba' && <div className="rounded-2xl surface-elevated p-5"><p className="text-sm font-semibold text-fg">Riwayat Lomba</p><p className="mt-1 text-sm text-fg-muted">{competitionCount} lomba tercatat untuk profil ini.</p></div>}{tab === 'statistik' && <div className="grid gap-3 sm:grid-cols-3"><div className="rounded-2xl surface-elevated p-4"><p className="text-xs text-fg-muted">XP</p><b className="text-lg text-fg">{Number(profile.total_xp || 0).toLocaleString('id-ID')}</b></div><div className="rounded-2xl surface-elevated p-4"><p className="text-xs text-fg-muted">EduCoin</p><b className="text-lg text-fg">{Number(profile.edu_coin || 0).toLocaleString('id-ID')}</b></div><div className="rounded-2xl surface-elevated p-4"><p className="text-xs text-fg-muted">Prestasi</p><b className="text-lg text-fg">{awards.length}</b></div></div>}{tab === 'badge' && (canShowCollection ? <ProfileBadgeCollection badges={badges} own={own}/> : <div className="py-10 text-center"><Filter className="mx-auto text-fg-muted" size={28}/><p className="mt-2 text-sm font-semibold text-fg">Koleksi badge disembunyikan</p><p className="mt-1 text-xs text-fg-muted">Pemilik profil memilih untuk tidak menampilkan koleksi badge.</p></div>)}{tab === 'kategori' && <div className="flex flex-wrap gap-2">{cats.map((c) => <span key={c} className="chip">{CATEGORY_LABELS[c] || c}</span>)}{!cats.length && <p className="text-sm text-fg-muted">Belum ada kategori favorit.</p>}</div>}</Card></div>
