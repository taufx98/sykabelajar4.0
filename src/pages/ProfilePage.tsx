...
function ProfileBadgeCollection({ badges, own }: { badges: ProfileBadge[]; own: boolean }) {
  ...
  return <div>
    <div className="flex flex-col gap-3 border-b surface-border pb-4 sm:flex-row sm:items-center sm:justify-between">
      ...
      <div className="grid grid-cols-3 gap-2 sm:flex"><select className="input min-w-[132px] pr-8 text-xs" value={category} onChange={(e) => setCategory(e.target.value)} aria-label="Filter kategori">{categories.map((value) => <option key={value} value={value}>{value === 'Semua' ? 'Semua Kategori' : value}</option>)}</select><select className="input min-w-[118px] pr-8 text-xs" value={rarity} onChange={(e) => setRarity(e.target.value)} aria-label="Filter rarity">{rarities.map((value) => <option key={value} value={value}>{value === 'Semua' ? 'Semua Rarity' : value}</option>)}</select><select className="input min-w-[104px] pr-8 text-xs" value={sort} onChange={(e) => setSort(e.target.value)} aria-label="Urutan badge"><option>Terbaru</option><option>Nama</option></select></div>
    </div>
  ...
  return <div className="min-h-screen bg-slate-50/40 pb-12 dark:bg-slate-950/20">
    <div className="px-3 pt-4 sm:px-5 md:px-8">
      <div className="mx-auto max-w-6xl overflow-hidden rounded-3xl border surface-border surface-card-bg shadow-sm">
        <div className="relative h-32 overflow-hidden sm:h-36 md:h-40 surface-bg">{profile.cover_url ? <img src={optimizedCloudinaryUrl(profile.cover_url, { width: 1600 })} alt="" className="h-full w-full object-cover"/> : <div className="h-full w-full bg-gradient-to-r from-sky-100 via-indigo-50 to-emerald-50 dark:from-slate-900 dark:via-slate-900 dark:to-slate-800"/>}<div className="pointer-events-none absolute inset-0 bg-gradient-to-t from-slate-950/75 via-slate-950/15 to-transparent"/></div>
