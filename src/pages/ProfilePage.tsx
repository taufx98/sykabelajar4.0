...
import { Award, Calendar, Edit2, Filter, GraduationCap, MessageCircle, School, Settings2, UserMinus, Users, X } from 'lucide-react';
    ...
    (async () => {
      ...
      try {
        ...
        const [a, s, u, b, v] = await Promise.all([
          ...
          getProfileBadges(p.data.id).catch(() => []),
  ...
  return <div className="min-h-screen pb-10">
      ...
      <div className="mx-auto max-w-5xl rounded-3xl border surface-border surface-card-bg p-5 md:p-6 shadow-sm">
        <div className="flex flex-col md:flex-row md:items-end gap-5">
          ...
          <div className="min-w-0 flex-1 text-center md:text-left">
            ...
            <div className="mt-4 flex flex-wrap justify-center md:justify-start gap-2">{own ? <><Link to="/profile/edit"><Button size="sm" variant="outline" icon={<Edit2 size={14}/>}>Edit Profil</Button></Link><Link to="/profile/interface-settings"><Button size="sm" variant="outline" icon={<Settings2 size={14}/>}>Tampilan & Privasi</Button></Link></> : user && <><Button size="sm" disabled={busy} variant={follow === 'approved' || follow === 'auto' ? 'outline' : 'primary'} onClick={() => void doFollow()}>{follow === 'pending' ? 'Diminta' : follow === 'approved' || follow === 'auto' ? 'Berhenti Mengikuti' : 'Ikuti'}</Button>{(follow === 'approved' || follow === 'auto') && <Link to={`/pesan?user_id=${profile.id}`}><Button size="sm" variant="outline" icon={<MessageCircle size={14}/>}>Kirim Pesan</Button></Link>}</>}</div>
          </div>
        </div>
        {canShowBadges && resolvedShowcase.length > 0 && <div className="mt-6 pt-5 border-t surface-border"><div className="flex flex-wrap justify-center md:justify-start gap-2">{resolvedShowcase.map((badge) => <div key={badge.id} className="inline-flex items-center gap-2 rounded-full border surface-border surface-elevated px-2.5 py-1.5"><div className="h-7 w-7 flex items-center justify-center"><img src={badge.icon_url ? optimizedCloudinaryUrl(badge.icon_url, { width: 64, height: 64 }) : undefined} alt="" className="max-h-7 max-w-7 object-contain"/>{!badge.icon_url && <span className="text-lg">{badge.icon_emoji || '🏅'}</span>}</div><span className="text-xs font-semibold text-fg">{badge.name}</span><span className="text-[10px] text-fg-muted">{badge.rarity}</span></div>)}</div></div>}
      </div>
    </div>
    ...
    <div className="px-4 md:px-8 mt-6"><div className="mx-auto max-w-5xl flex justify-center overflow-x-auto border-b surface-border">{(['prestasi','lomba','statistik','badge','kategori'] as Tab[]).map((t) => <button key={t} onClick={() => setTab(t)} className={`px-4 py-3 text-sm font-medium whitespace-nowrap border-b-2 ${tab === t ? 'text-accent border-accent' : 'text-fg-muted border-transparent hover:text-fg'}`}>{t === 'prestasi' ? 'Prestasi' : t === 'lomba' ? 'Lomba' : t === 'statistik' ? 'Statistik' : t === 'badge' ? 'Badge' : 'Kategori Favorit'}</button>)}</div>
      <Card className="mx-auto max-w-5xl mt-4 p-5 md:p-6">
        {tab === 'prestasi' && <div className="space-y-2">{awards.slice(0, 10).map((a) => <div key={a.id} className="flex items-center gap-3 rounded-xl p-2 hover:surface-elevated"><Award size={18} className="text-accent"/><div><p className="font-semibold text-fg">{a.title}</p><p className="text-xs text-fg-muted">{a.subtitle || a.rank_code || 'Penghargaan'}</p></div></div>)}{!awards.length && <p className="text-sm text-fg-muted">Belum ada prestasi.</p>}</div>}
