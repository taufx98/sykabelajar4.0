...
export function OrganizerGradingPage() {
  ...
  return <div className="min-h-screen surface-bg text-fg-secondary p-5 md:p-8"><div className="max-w-6xl mx-auto space-y-5">
    <div><p className="text-xs text-moss-400 font-semibold">PENYELENGGARA · GRADING</p><h1 className="font-display text-2xl md:text-3xl font-bold text-fg">Penilaian Manual</h1><p className="text-sm text-slate-500 mt-1">Nilai essay/file disimpan sebagai grading item di Supabase sebelum hasil difinalisasi.</p></div>
    ...
    <div className="grid lg:grid-cols-[320px_1fr] gap-5">
      ...
      <Card className="p-5">{!detail?<div className="py-16 text-center text-slate-500">Pilih attempt untuk mulai menilai.</div>:<><div className="flex items-center justify-between gap-3 mb-5"><div><p className="text-sm text-fg font-semibold">Attempt {detail.attempt.id.slice(0,12)}</p><p className="text-xs text-slate-500">{detail.items.length} item · skor sementara {total}</p></div><Button loading={busy} onClick={()=>void finalize()} icon={<CheckCircle2 size={15}/>}>Finalisasi</Button></div><div className="space-y-4">{detail.items.map((item:any,index:number)=><Card key={item.id} className="p-4 bg-white/[.02]"><div className="flex items-start gap-3"><div className="w-7 h-7 rounded-lg bg-moss-500/10 text-moss-300 flex items-center justify-center text-xs font-bold">{index+1}</div><div className="flex-1 min-w-0"><p className="text-sm text-white whitespace-pre-wrap">{item.question?.prompt || 'Soal'}</p><p className="text-xs text-slate-500 mt-1">{item.question?.type} · maksimal {item.question?.points ?? 0} poin</p><div className="mt-3 p-3 rounded-xl surface-card-bg border surface-border text-sm text-fg-secondary whitespace-pre-wrap break-words">{item.answer?.value ?? (item.answer ? JSON.stringify(item.answer) : 'Belum ada jawaban')}</div><div className="grid md:grid-cols-[160px_1fr_auto] gap-2 mt-3"><input className="input" type="number" min="0" max={Number(item.question?.points ?? 0)} step="0.1" placeholder="Nilai" value={scores[item.id] ?? ''} onChange={e=>setScores(s=>({...s,[item.id]:e.target.value}))}/><input className="input" placeholder="Feedback untuk peserta" value={feedback[item.id] ?? ''} onChange={e=>setFeedback(s=>({...s,[item.id]:e.target.value}))}/><Button size="sm" loading={saving===item.id} onClick={()=>void saveItem(item)} icon={<Save size={14}/>}>Simpan</Button></div></div></div></Card>)}</div></>}</Card>
    </div>
  </div></div>;
}
