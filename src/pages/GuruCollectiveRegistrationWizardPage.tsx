...
import { listCollectiveCompetitions, listTeacherRosters, listRosterStudents, getCollectiveRegistrationOptions, quoteCollectiveRegistration, createCollectiveRegistrationOrder, getCollectiveRegistration, provisionCollectiveRegistration, type RosterStudent, type TeacherRoster, type CollectiveRegistrationOptions, type CollectiveRegistrationQuote } from '@/services/collectiveParticipant.service';

const STEPS = ['Lomba & identitas', 'Pilih siswa', 'Kategori & sertifikat', 'Review & pembayaran', 'Selesai'];

type Credential = { participant_code: string; temporary_password: string; full_name: string };

export function GuruCollectiveRegistrationWizardPage() {
  const { user } = useApp();
  const [step, setStep] = useState(1);
  const [competitions, setCompetitions] = useState<Array<{ id: string; title: string; participant_mode: string; status: string; starts_at: string | null; ends_at: string | null }>>([]);
  ...
  useEffect(() => {
    void Promise.all([listCollectiveCompetitions(), listTeacherRosters(), listOrganizerPaymentMethods()]).then(([competitionRows, rosterRows, methods]) => {
      setCompetitions(competitionRows);
      setRosters(rosterRows);
      setPaymentMethods(methods);
    }).catch((e) => toast.error(e?.message || 'Gagal memuat data pendaftaran kolektif.')).finally(() => setLoadingCompetitions(false));
  }, []);
  ...
  useEffect(() => {
    if (!competitionId) { setOptions(null); setQuote(null); return; }
    setBusy(true);
    void getCollectiveRegistrationOptions(competitionId).then((data) => {
      setOptions(data);
      setCompetitionLevelId(data.levels[0]?.id ?? null);
      setCertificatePolicy(data.certificate_policies[0] ?? 'EVENT_DEFAULT');
    }).catch((e) => toast.error(e?.message || 'Gagal memuat konfigurasi lomba.')).finally(() => setBusy(false));
  }, [competitionId]);
  ...
  const buildQuote = async () => {
    setBusy(true);
    try { setQuote(await quoteCollectiveRegistration(competitionId, selectedStudentIds, certificatePolicy)); }
    catch (e: any) { toast.error(e?.message || 'Gagal menghitung harga.'); throw e; }
  ...
  const uploadProof = async (file: File) => {
    ...
    setBusy(true);
    try {
      const r = await uploadImage(file, { folder: 'sykabelajar/collective-registration/proofs' });
      setProof({ url: r.secure_url, publicId: r.public_id, width: r.width, height: r.height, version: r.version, resourceType: r.resource_type });
      toast.success('Bukti pembayaran berhasil diupload.');
    } catch (e: any) { setProofFile(null); toast.error(e?.message || 'Upload bukti pembayaran gagal.'); }
    finally { setBusy(false); }
  };

  const submit = async () => {
    if (!quote) return toast.error('Total pendaftaran belum dihitung.');
    ...
    try {
      ...
      setStep(5);
    } catch (e: any) { toast.error(e?.message || 'Pendaftaran kolektif gagal dibuat.'); }
    finally { setBusy(false); }
  };
  ...
  const refreshStatus = async () => {
    ...
    try {
      ...
      if (String(current.order_status) === 'PAID' || String(current.status) === 'PAID' || String(current.status) === 'PROVISIONED') {
        ...
        setCredentials(rows.map((r) => ({ participant_code: r.participant_code, temporary_password: r.temporary_password, full_name: r.full_name })));
        toast.success('Pembayaran sudah dikonfirmasi. Akses peserta siap.');
      } else toast.error('Pesanan masih menunggu verifikasi pembayaran.');
    } catch (e: any) { toast.error(e?.message || 'Gagal memperbarui status.'); }
    finally { setBusy(false); }
  };
  ...
  const next = async () => {
    if (step === 1) {
      ...
      if (!rosterId) return toast.error('Pilih roster siswa.');
    ...
    if (step === 3) { try { await buildQuote(); } catch { return; } }
    if (step === 4) return void submit();
  ...
  return <div className="min-h-screen surface-bg p-5 md:p-8"><div className="max-w-5xl mx-auto space-y-5">
    ...
    <div><p className="text-xs text-accent font-semibold tracking-wider">COLLECTIVE REGISTRATION</p><h1 className="text-2xl md:text-3xl font-bold text-fg mt-1">Daftarkan siswa ke lomba</h1><p className="text-sm text-fg-muted mt-1">Wizard pendaftaran kolektif dengan validasi roster, harga, pembayaran, dan provisioning di server.</p></div>
