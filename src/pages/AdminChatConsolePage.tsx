    ...
    <div className="fixed inset-0 z-[150] flex items-center justify-center bg-black/70 p-4">
      ...
      <div className="relative flex h-[80vh] w-full max-w-3xl flex-col overflow-hidden card">
        <header className="flex shrink-0 items-center gap-3 border-b surface-border px-5 py-4">
          ...
          <button className="rounded-lg p-2 text-fg-muted hover:bg-white/5" onClick={onClose} aria-label="Tutup"><X size={18} /></button>
        </header>
            ...
            return (
              <div key={message.id} className={`flex ${mine ? 'justify-end' : 'justify-start'}`}>
                <div className={`max-w-[78%] rounded-2xl px-4 py-2.5 text-sm ${mine ? 'rounded-br-sm bg-moss-600 text-white' : 'rounded-bl-sm surface-elevated text-fg'}`}>
                  {index === 0 && !mine ? <TicketBubble row={{ ...row, id: row.thread_id, participant_id: null, status: row.status as 'open' | 'closed', rating: row.rating, closed_at: row.closed_at, created_at: row.created_at, thread_type: 'ticket' }} body={message.body} /> : <p className="whitespace-pre-wrap break-words">{message.body}</p>}
