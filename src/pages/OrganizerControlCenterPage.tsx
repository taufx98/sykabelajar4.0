...
function humanStatus(value?: string | null) {
  return String(value ?? '').replace(/_/g, ' ').toLowerCase().replace(/\b\w/g, (m: string) => m.toUpperCase()) || '—';
}
...
export function OrganizerControlCenterPage() {
  ...
  const activeTab: TabKey = (TABS.some(([key]) => key === rawTab) ? rawTab : 'overview') as TabKey;
