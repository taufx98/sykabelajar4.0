...
import { joinCollectiveChatGroup, listCollectiveChatGroupMembers, listCollectiveChatGroups, loadCollectiveChatGroupMessages, markCollectiveChatGroupRead, sendCollectiveChatGroupMessage, subscribeCollectiveGroupChat } from '@/services/collectiveGroupChat.service';
...
function mapRealtimeMessage(row: ChatGroupMessage, members: ChatGroupMember[]) {
  const member = row.sender_collective_participant_id
    ...
    : row.sender_user_id
      ...
      : null;
  return { ...row, sender_name: member?.display_name || (row.sender_collective_participant_id ? 'Peserta' : 'Pengguna'), sender_role: row.sender_collective_participant_id ? 'PESERTA' : member?.member_role || 'MEMBER' } as ChatGroupMessage;
}

function extractInviteToken(raw: string) {
  const value = raw.trim();
  if (!value) return '';
  try {
    const url = new URL(value);
    return url.searchParams.get('join_group')?.trim() || '';
  } catch {
    return value;
  }
}

function isCollectiveSessionError(error: unknown) {
  return error instanceof Error && error.message.toLowerCase().includes('sesi peserta kolektif');
}
...
export function CollectiveMessagesPage() {
  ...
  const membersRef = useRef<ChatGroupMember[]>([]);
  ...
  const loadGroups = useCallback(async () => {
    if (!accessToken) { navigate('/peserta-kolektif/login', { replace: true }); return []; }
    try {
      ...
      if (isCollectiveSessionError(error)) navigate('/peserta-kolektif/login', { replace: true });
      return [];
    } finally { setLoading(false); }
  }, [accessToken, navigate]);

  useEffect(() => { if (!accessToken) { navigate('/peserta-kolektif/login', { replace: true }); return; } void loadGroups(); }, [accessToken, loadGroups, navigate]);

  const applyJoinToken = useCallback(async (raw: string) => {
    const inviteToken = extractInviteToken(raw);
    if (!inviteToken || joinBusy) return;
    setJoinBusy(true);
    try {
      const result = await joinCollectiveChatGroup(accessToken, inviteToken);
      if (!result.ok) throw new Error('GROUP_INVITE_INVALID');
      const rows = await loadGroups();
      const target = rows.find((group) => group.id === result.group_id) || null;
      if (target) { setSelectedGroup(target); setMobileList(false); }
      setJoinValue('');
      setJoinOpen(false);
      toast.success('Berhasil bergabung ke group.');
    } catch (error: unknown) { toast.error(error instanceof Error ? error.message : 'Undangan tidak valid.'); }
    finally { setJoinBusy(false); }
  }, [accessToken, joinBusy, loadGroups]);
  ...
  useEffect(() => {
    ...
    void joinCollectiveChatGroup(accessToken, inviteToken)
      ...
      .catch((error: unknown) => { if (alive) toast.error(error instanceof Error ? error.message : 'Link group tidak valid.'); })
      .finally(() => {
        if (!alive) return;
  ...
  useEffect(() => {
    ...
    let alive = true;
    setMessages([]); setMembers([]); membersRef.current = [];
    void Promise.all([
      ...
      if (!alive) return;
      setMessages(rows); setMembers(memberRows); membersRef.current = memberRows; setHasOlder(rows.length >= PAGE);
    ...
    }).catch((error: unknown) => { if (alive) toast.error(error instanceof Error ? error.message : 'Gagal memuat pesan group.'); });
    ...
    const unsubscribePromise = subscribeCollectiveGroupChat({
      ...
      onInsert: (row) => {
        if (!alive) return;
        const message = mapRealtimeMessage(row, membersRef.current);
      ...
      onError: (error) => { if (alive) toast.error(error.message); },
    });
    return () => { alive = false; void unsubscribePromise.then((unsubscribe) => unsubscribe()); };
  }, [accessToken, selectedGroup?.id]);

  const chooseGroup = async (group: ChatGroup) => {
    setSelectedGroup(group); setInput(''); setMobileList(false);
  ...
  const send = async () => {
    ...
    try {
      ...
      setInput('');
      const member = membersRef.current.find((item) => item.collective_participant_id === participantId);
      ...
      requestAnimationFrame(() => { if (scrollRef.current) scrollRef.current.scrollTop = scrollRef.current.scrollHeight; });
    } catch (error: unknown) { toast.error(error instanceof Error ? error.message : 'Pesan gagal dikirim.'); }
  ...
  const loadOlder = async () => {
    ...
    } catch (error: unknown) { toast.error(error instanceof Error ? error.message : 'Gagal memuat pesan lama.'); }
  ...
  return <div className="h-[calc(100vh-56px)] md:h-screen overflow-hidden surface-bg p-2 md:p-4">
    <div className="h-full max-w-7xl mx-auto flex flex-col">
      <div className="shrink-0 flex items-center justify-between pb-2"><Link to="/peserta-kolektif" className="inline-flex items-center gap-2 text-xs text-fg-muted"><ArrowLeft size={14}/>Portal Peserta</Link><div className="flex items-center gap-2 text-xs text-fg-muted"><Avatar name={String(participant?.full_name || 'Peserta')} id={participantId || 'collective'} size={30} src={participant?.photo_url ? String(participant.photo_url) : undefined}/><span className="hidden sm:block">{String(participant?.full_name || 'Peserta')}</span></div></div>
      ...
      <div className="flex-1 min-h-0 grid md:grid-cols-[320px_1fr] rounded-2xl overflow-hidden border surface-border surface-card-bg">
        <aside className={`${mobileList ? 'flex' : 'hidden'} md:flex flex-col min-h-0 border-r surface-border`}><div className="p-3 border-b surface-border"><div className="grid grid-cols-2 rounded-xl bg-black/10 p-1 gap-1"><button type="button" disabled className="rounded-lg px-3 py-2 text-xs font-semibold text-fg-muted opacity-50 cursor-not-allowed">Chat Pribadi</button><button type="button" className="rounded-lg px-3 py-2 text-xs font-semibold bg-white/10 text-fg shadow-sm">Chat Grup</button></div><div className="mt-3"><Button fullWidth size="sm" variant="outline" onClick={() => setJoinOpen(true)} icon={<Link2 size={14}/>}>Gabung Group</Button></div></div><div className="flex-1 overflow-y-auto p-2">{loading ? <p className="py-10 text-center text-xs text-fg-muted">Memuat group…</p> : groups.length ? groups.map((group) => <button key={group.id} type="button" onClick={() => void chooseGroup(group)} className={`w-full text-left flex items-center gap-3 p-3 rounded-xl border ${selectedGroup?.id === group.id ? 'border-moss-500/50 bg-moss-500/10' : 'border-transparent hover:bg-white/5'}`}><div className="w-9 h-9 rounded-xl bg-accent/10 text-accent flex items-center justify-center shrink-0"><Users size={17}/></div><div className="min-w-0 flex-1"><div className="flex items-center gap-2"><p className="text-sm font-medium text-fg truncate">{group.name}</p>{group.unread_count > 0 && <span className="min-w-5 h-5 rounded-full bg-red-500 text-white text-[9px] flex items-center justify-center">{group.unread_count > 99 ? '99+' : group.unread_count}</span>}</div><p className="text-[10px] text-fg-muted truncate">{group.member_count} anggota · {group.last_message || 'Belum ada pesan'}</p></div></button>) : <div className="py-10 text-center px-4"><p className="text-sm text-fg-muted">Belum ada group.</p><p className="text-xs text-fg-muted mt-1">Gunakan link undangan dari Guru untuk bergabung.</p></div>}</div></aside>
        <section className={`${mobileList ? 'hidden' : 'flex'} md:flex flex-col min-h-0 min-w-0 relative`}>{!selectedGroup ? <div className="flex-1 flex items-center justify-center text-sm text-fg-muted">Pilih group.</div> : <><header className="shrink-0 p-3 border-b surface-border flex items-center gap-3"><button type="button" className="md:hidden p-2 rounded-lg hover:bg-white/5" onClick={() => setMobileList(true)}><ArrowLeft size={18}/></button><div className="w-10 h-10 rounded-xl bg-accent/10 text-accent flex items-center justify-center"><Users size={18}/></div><div className="min-w-0 flex-1"><p className="text-sm font-semibold text-fg truncate">{selectedGroup.name}</p><p className="text-[11px] text-fg-muted truncate">{selectedGroup.member_count} anggota · {selectedGroup.group_type === 'event' ? 'Event Group' : selectedGroup.group_type === 'class' ? 'Group Kelas' : 'Group Umum'}</p></div></header><div ref={scrollRef} onScroll={(event) => { if (event.currentTarget.scrollTop <= 30) void loadOlder(); }} className="flex-1 overflow-y-auto p-4 space-y-3">{olderBusy && <p className="text-[10px] text-fg-muted text-center">Memuat pesan lama…</p>}{messages.length ? messages.map((message) => { const mine = message.sender_collective_participant_id === participantId; return <div key={message.id} className={`flex ${mine ? 'justify-end' : 'justify-start'}`}><div className={`max-w-[78%] rounded-2xl px-4 py-2.5 text-sm ${mine ? 'bg-moss-600 text-white rounded-br-sm' : 'surface-elevated text-fg rounded-bl-sm'}`}><div className="flex items-center gap-2 mb-1"><span className="text-[10px] font-semibold opacity-75">{mine ? 'Kamu' : message.sender_name}</span><Badge color="moss">{message.sender_role}</Badge></div><p className="whitespace-pre-wrap break-words">{message.body}</p><p className="text-[9px] mt-1 opacity-60 text-right">{formatTime(message.created_at)}</p></div></div>; }) : <p className="text-sm text-fg-muted text-center py-10">Belum ada pesan.</p>}</div><div className="shrink-0 p-3 border-t surface-border"><div className="flex items-end gap-2"><textarea value={input} onChange={(event) => setInput(event.target.value)} onKeyDown={(event) => { if (event.key === 'Enter' && !event.shiftKey) { event.preventDefault(); void send(); } }} placeholder="Tulis pesan ke group…" maxLength={2000} className="input flex-1 min-h-[42px] max-h-32 resize-none" rows={1}/><button type="button" className="p-2.5 rounded-xl bg-moss-600 text-white disabled:opacity-40" disabled={busy || !input.trim()} onClick={() => void send()}><Send size={17}/></button></div></div></>}</section>
      </div>
    </div>
    {joinOpen && <div className="fixed inset-0 z-[170] flex items-center justify-center p-4"><button type="button" aria-label="Tutup" className="absolute inset-0 bg-black/70" onClick={() => { if (!joinBusy) setJoinOpen(false); }}/><div className="relative w-full max-w-md card p-5"><div className="flex items-center justify-between"><div><h3 className="font-semibold text-fg">Gabung Group</h3><p className="text-xs text-fg-muted mt-1">Tempel link undangan dari Guru.</p></div><button type="button" onClick={() => { if (!joinBusy) setJoinOpen(false); }} className="p-1.5 text-fg-muted hover:text-fg" aria-label="Tutup">×</button></div><input autoFocus value={joinValue} disabled={joinBusy} className="input w-full mt-4" placeholder="https://sykabelajar.my.id/pesan?join_group=…" onChange={(event) => setJoinValue(event.target.value)} onKeyDown={(event) => { if (event.key === 'Enter') void applyJoinToken(event.currentTarget.value); }}/><Button fullWidth className="mt-3" loading={joinBusy} disabled={!joinValue.trim()} onClick={() => void applyJoinToken(joinValue)} icon={<Link2 size={14}/>}>Gabung</Button></div></div>}
  </div>;
}
