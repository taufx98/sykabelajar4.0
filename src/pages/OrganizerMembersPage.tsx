...
export function OrganizerMembersPage() {
  ...
  return <div className="min-h-screen surface-bg text-fg-secondary p-5 md:p-8"><div className="max-w-6xl mx-auto"><Link to="/organizer" className="inline-flex items-center gap-2 text-xs text-slate-500 hover:text-fg mb-5"><ArrowLeft size={14}/> Kembali</Link><div className="flex items-center justify-between mb-6"><div><p className="text-xs text-accent">ORGANIZER CONTROL</p><h1 className="text-2xl font-bold text-fg">Manajemen Member</h1><p className="text-sm text-slate-500 mt-1">{org.name} · akses dikelola dari organizer_members.</p></div><Badge color="moss"><Users size={14}/> {members.length}</Badge></div>
    ...
    <div className="space-y-2">{members.map((m) => { const p = profiles[m.user_id]; return <Card key={m.id} className="p-4 flex items-center gap-3"><div className="w-10 h-10 rounded-lg overflow-hidden bg-moss-500/10">{p?.avatar_url ? <img src={p.avatar_url} className="w-full h-full object-cover" alt=""/> : <div className="h-full flex items-center justify-center font-bold text-accent">{String(p?.full_name || p?.username || '?').slice(0,1).toUpperCase()}</div>}</div><div className="flex-1"><p className="font-semibold text-fg">{p?.full_name || p?.username || m.user_id.slice(0,8)}</p><p className="text-xs text-slate-500">@{p?.username || '—'} · {p?.institution || '—'}</p></div><select className="input w-36" value={m.role} disabled={busy} onChange={async (e) => { setBusy(true); try { const { error } = await supabase.from('organizer_members').update({ role: e.target.value }).eq('id', m.id); if (error) throw error; await load(); } catch (err: any) { toast.error(err.message); } finally { setBusy(false); } }}><option value="owner">Owner</option><option value="editor">Editor</option><option value="reviewer">Reviewer</option><option value="finance">Finance</option><option value="viewer">Viewer</option></select><Badge color={m.status === 'ACTIVE' ? 'moss' : 'default'}>{m.status}</Badge><button disabled={busy} onClick={() => void updateStatus(m.id, m.status === 'ACTIVE' ? 'SUSPENDED' : 'ACTIVE')} className="p-2 rounded-lg hover:bg-surface-elevated/50"><Shield size={16}/></button><button disabled={busy} onClick={() => void removeMember(m.id)} className="p-2 text-red-400 rounded-lg hover:bg-red-500/10"><Trash2 size={16}/></button></Card>; })}{!members.length && <Card className="p-8 text-center text-slate-500">Belum ada member tambahan.</Card>}</div>
  </div></div>;
}
