...
import { LayoutDashboard, Trophy, Users, ShoppingBag, FileText, Store, Settings, ShieldCheck, Search, Trash2, Plus, Edit3, ExternalLink } from 'lucide-react';
...
import { banUser, deleteCompetition, deletePost, deleteProduct, loadAdminCore, saveCompetition, savePost, saveProduct, setUserRole, transitionCompetition } from '@/services/adminCore.service';
...
export function AdminPage() {
  ...
  const setRole = async (id: string, role: BackendRole) => await run(async () => { const saved = await setUserRole(id, role); if (saved) setUsers(rows => rows.map(row => row.id === id ? { ...row, role: saved.role ?? role } : row)); }, 'Role pengguna diperbarui.', 'Gagal memperbarui role.');
...
function Field({ label, value, onChange, textarea, type='text' }: any) { return <label className="block"><span className="label">{label}</span>{textarea?<textarea className="input min-h-32" value={value} onChange={e=>onChange(e.target.value)}/>:<input className="input" type={type} value={value} onChange={e=>onChange(e.target.value)}/>}</label>; }
