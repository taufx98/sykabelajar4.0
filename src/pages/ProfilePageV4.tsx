...
import { Award, Calendar, Edit2, GraduationCap, MessageCircle, School, Settings2, UserMinus, Users, X } from 'lucide-react';
...
function ProfilePortrait({ name, id, src }: { name: string; id: string; src?: string | null }) {
  ...
  if (src) {
    return <img src={optimizedCloudinaryUrl(src, { width: 336, height: 448, crop: 'fill' })} alt={name} width={168} height={224} className={common} />;
  }
...
export function ProfilePageV4() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        const sr = Array.isArray(s.data) ? s.data[0] : s.data;
        const ui = Array.isArray(u.data) ? u.data[0] : u.data;
        let fs: FollowStatus = 'none';
        if (user && user.id !== p.data.id) fs = await getFollowStatus(user.id, p.data.id).catch(() => 'none' as FollowStatus);
        if (alive) {
          setProfile(p.data);
          setAwards(a.data || []);
  ...
  const showcase = Array.isArray(profile?.badge_showcase) && profile.badge_showcase.length ? profile.badge_showcase.slice(0, 3) : badges.slice(0, 3);
  ...
  return <div className="min-h-screen pb-10">
    <div className="h-32 md:h-44 overflow-hidden surface-bg">{profile.cover_url && <img src={profile.cover_url} alt="" className="w-full h-full object-cover"/>}</div>
    <div className="relative -mt-14 px-4 text-center">
      <div className="flex justify-center"><ProfilePortrait name={profile.full_name || profile.username} id={profile.id} src={profile.avatar_url || undefined}/></div>
      <div className="mt-4 flex flex-wrap items-baseline justify-center gap-1.5"><h1 className="font-display text-xl md:text-2xl font-bold text-fg">{profile.full_name || profile.username}</h1><VerifiedMark type={profile.verification_type}/><span className="text-sm md:text-base text-fg-muted font-medium">/ @{profile.username}</span></div>
      <div className="mt-3 flex flex-wrap justify-center gap-4 text-[11px] text-fg-muted">{profile.institution && <span className="flex items-center gap-1"><School size={12}/>{profile.institution}</span>}{grade && <span className="flex items-center gap-1"><GraduationCap size={12}/>{grade}</span>}{profile.birth_date && <span className="flex items-center gap-1"><Calendar size={12}/>Lahir {new Date(profile.birth_date).toLocaleDateString('id-ID', { day: 'numeric', month: 'long', year: 'numeric' })}</span>}</div>
      {showcase.length > 0 && <div className="mt-3 flex justify-center gap-2">{showcase.map((b: any, i: number) => <span className="chip" key={i}>{String(b)}</span>)}</div>}
      <div className="mt-4 flex justify-center gap-8"><button type="button" disabled={!showFollowing} onClick={() => showFollowing && setSocial('following')} className="text-sm hover:text-accent disabled:opacity-50"><b>{following}</b> <span className="text-fg-muted">Mengikuti</span></button><button type="button" disabled={!showSocial} onClick={() => showSocial && setSocial('followers')} className="text-sm hover:text-accent disabled:opacity-50"><b>{followers}</b> <span className="text-fg-muted">Penggemar</span></button></div>
      <div className="mt-4 flex flex-wrap justify-center gap-2">{own ? <><Link to="/profile/edit"><Button size="sm" variant="outline" icon={<Edit2 size={14}/>}>Edit Profil</Button></Link><Link to="/profile/interface-settings"><Button size="sm" variant="outline" icon={<Settings2 size={14}/>}>Tampilan & Privasi</Button></Link></> : user && <><Button size="sm" disabled={busy} variant={follow === 'approved' || follow === 'auto' ? 'outline' : 'primary'} onClick={() => void doFollow()}>{follow === 'pending' ? 'Diminta' : follow === 'approved' || follow === 'auto' ? 'Berhenti Mengikuti' : 'Ikuti'}</Button>{(follow === 'approved' || follow === 'auto') && <Link to={`/pesan?user_id=${profile.id}`}><Button size="sm" variant="outline" icon={<MessageCircle size={14}/>}>Kirim Pesan</Button></Link>}</>}</div>
    </div>
    <div className="px-4 md:px-8 mt-6"><div className="mx-auto max-w-5xl flex justify-center overflow-x-auto border-b surface-border">{(['prestasi','lomba','statistik','badge','kategori'] as Tab[]).map(t => <button key={t} onClick={() => setTab(t)} className={`px-4 py-3 text-sm font-medium whitespace-nowrap ${tab === t ? 'text-accent' : 'text-fg-muted'}`}>{t === 'prestasi' ? 'Prestasi' : t === 'lomba' ? 'Lomba' : t === 'statistik' ? 'Statistik' : t === 'badge' ? 'Badge' : 'Kategori Favorit'}</button>)}</div>
      <Card className="mt-4 p-5">{tab === 'prestasi' && <div className="space-y-2">{awards.slice(0, 10).map(a => <div key={a.id} className="flex items-center gap-3"><Award size={18} className="text-accent"/><div><p className="font-semibold text-fg">{a.title}</p><p className="text-xs text-fg-muted">{a.subtitle || a.rank_code || 'Penghargaan'}</p></div></div>)}{!awards.length && <p className="text-sm text-fg-muted">Belum ada prestasi.</p>}</div>}{tab === 'lomba' && <p className="text-sm text-fg-muted">Riwayat lomba tersedia di profil.</p>}{tab === 'statistik' && <div className="grid md:grid-cols-3 gap-3"><div><p className="text-xs text-fg-muted">XP</p><b>{Number(profile.total_xp || 0).toLocaleString('id-ID')}</b></div><div><p className="text-xs text-fg-muted">EduCoin</p><b>{Number(profile.edu_coin || 0).toLocaleString('id-ID')}</b></div><div><p className="text-xs text-fg-muted">Prestasi</p><b>{awards.length}</b></div></div>}{tab === 'badge' && <div className="grid sm:grid-cols-3 gap-2">{badges.map((b: any, i: number) => <div key={i} className="rounded-xl border surface-border p-3 text-sm text-fg">{String(b)}</div>)}</div>}{tab === 'kategori' && <div className="flex flex-wrap gap-2">{cats.map(c => <span key={c} className="chip">{CATEGORY_LABELS[c] || c}</span>)}</div>}</Card></div>
