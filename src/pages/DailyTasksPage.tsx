...
export function DailyTasksPage() {
  ...
  return <div>
    <div className="sticky top-0 z-20 glass border-b surface-border px-4 py-3">
      <h2 className="font-display font-bold text-lg text-fg">Daily Tasks</h2>
    ...
    <div className="p-4 space-y-4">
      <Card className="p-4 bg-gradient-to-r from-amber-500/15 to-transparent border-amber-500/20">
        <div className="flex items-center gap-4"><div className="w-14 h-14 rounded-2xl bg-amber-500/20 flex items-center justify-center"><Flame size={28} className="text-amber-400"/></div><div className="flex-1"><p className="text-sm font-semibold text-fg">Aktivitas Harian</p><p className="text-xs text-slate-400">Streak hanya ditampilkan jika backend memiliki catatan aktivitas.</p></div><div className="text-right"><p className="text-2xl font-bold text-fg">{completedCount}</p><p className="text-[10px] text-slate-500">selesai</p></div></div>
      </Card>
      ...
      {!loading && !error && tasks.map(task => { const expires = task.endsAt ? new Date(task.endsAt) : null; return <Card key={task.id} className={`p-4 ${task.completed ? 'opacity-60' : ''}`}>
        <div className="flex items-start gap-3"><div className="w-10 h-10 rounded-xl surface-elevated flex items-center justify-center shrink-0"><CalendarCheck size={18} className="text-moss-400"/></div><div className="flex-1 min-w-0"><div className="flex items-center gap-2 mb-1"><h3 className="font-semibold text-sm text-fg">{task.title}</h3>{task.completed&&<Badge color="moss"><Check size={10}/> Selesai</Badge>}</div><p className="text-xs text-slate-400 mb-2">{task.description || 'Aktivitas harian SykaBelajar.'}</p><div className="flex items-center gap-3 flex-wrap"><span className="text-xs text-moss-300 font-semibold">+{task.exp} XP</span><span className="text-xs text-amber-300 font-semibold">+{task.points} Coin</span>{expires&&<span className="text-xs text-slate-500 flex items-center gap-1"><Clock size={11}/> Sampai {expires.toLocaleString('id-ID')}</span>}</div></div></div>
