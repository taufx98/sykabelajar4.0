...
export function OrganizerQuestionEditorPage() {
  ...
  return <div className="min-h-screen surface-bg text-fg-secondary p-5 md:p-8">
    <div className="max-w-5xl mx-auto">
      <Link to="/organizer" className="inline-flex items-center gap-2 text-xs text-slate-500 hover:text-fg mb-5"><ArrowLeft size={14}/> Kembali ke Penyelenggara</Link>
      <div className="flex items-center justify-between mb-6">
        <div><p className="text-xs text-accent">BANK SOAL LIVE</p><h1 className="text-2xl font-bold text-fg">{loading ? 'Memuat…' : bank?.name || 'Bank Soal'}</h1><p className="text-sm text-slate-500 mt-1">Kelola pertanyaan dan kunci jawaban langsung di Supabase.</p></div>
