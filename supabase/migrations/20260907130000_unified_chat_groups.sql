...
create index if not exists idx_chat_group_invites_group on public.chat_group_invites(group_id, created_at desc);
...
returns boolean language sql stable security definer set search_path=public as $$
  select exists(select 1 from public.chat_group_members m where m.group_id=p_group_id and m.user_id=p_uid and m.status='ACTIVE');
$$;

create or replace function private.chat_group_user_is_admin(p_group_id uuid, p_uid uuid)
returns boolean language sql stable security definer set search_path=public as $$
  select exists(select 1 from public.chat_group_members m where m.group_id=p_group_id and m.user_id=p_uid and m.status='ACTIVE' and m.role in ('OWNER','ADMIN'));
$$;
...
declare
  v_uid uuid:=auth.uid(); v_group public.chat_groups; v_token text;
begin
...
from public.chat_groups g join public.chat_group_members gm on gm.group_id=g.id
where gm.user_id=auth.uid() and gm.status='ACTIVE' order by coalesce((select max(m.created_at) from public.chat_group_messages m where m.group_id=g.id),g.updated_at) desc;
$$;
...
and (private.chat_group_user_is_member(p_group_id,auth.uid()) or exists(select 1 from public.chat_groups g where g.id=p_group_id and g.owner_user_id=auth.uid()));
$$;
...
select m.id,m.sender_user_id,m.sender_collective_participant_id,
 ...
 case when m.sender_collective_participant_id is not null then 'PESERTA' when gm.role='OWNER' then 'GURU' when gm.role='ADMIN' then 'ADMIN' else 'MEMBER' end,
 m.body,m.created_at
...
where m.group_id=p_group_id and private.chat_group_user_is_member(p_group_id,auth.uid()) and (p_before is null or m.created_at<p_before)
...
returns public.chat_group_messages language plpgsql security definer set search_path=public,private as $$
declare v_uid uuid:=auth.uid(); v_group public.chat_groups; v_row public.chat_group_messages; v_body text:=trim(coalesce(p_body,''));
begin
 if v_uid is null then raise exception 'UNAUTHORIZED'; end if;
 ...
 if not found or not private.chat_group_user_is_member(p_group_id,v_uid) then raise exception 'ACCESS_DENIED'; end if;
...
declare v_uid uuid:=auth.uid(); v_group public.chat_groups; v_member public.chat_group_members; v_comp uuid;
begin
 if v_uid is null then raise exception 'UNAUTHORIZED'; end if;
 if not private.chat_group_user_is_admin(p_group_id,v_uid) then raise exception 'ACCESS_DENIED'; end if;
 ...
 insert into public.chat_group_members(group_id,user_id,collective_participant_id,role,status) values(p_group_id,p_user_id,p_collective_participant_id,'MEMBER','ACTIVE')
 on conflict do update set status='ACTIVE',role='MEMBER' returning * into v_member;
 return v_member;
end;$$;
...
begin
 ...
 update public.chat_group_members set status='REMOVED' where id=p_member_id and group_id=p_group_id and role<>'OWNER';
 return found;
end;$$;
...
begin
 ...
 if nullif(trim(p_name),'') is null then raise exception 'GROUP_NAME_REQUIRED'; end if;
 update public.chat_groups set name=trim(p_name),updated_at=now() where id=p_group_id returning * into v_group;
 if not found then raise exception 'GROUP_NOT_FOUND'; end if;
 return v_group;
end;$$;
...
declare v_uid uuid:=auth.uid(); v_group public.chat_groups; v_hash text:=encode(digest(trim(coalesce(p_invite_token,'')),'sha256'),'hex');
begin
 if v_uid is null then raise exception 'UNAUTHORIZED'; end if;
 ...
 if v_group.group_type='event' and not exists(select 1 from public.collective_participants cp where cp.competition_id=v_group.competition_id and cp.managed_by_user_id=v_uid and cp.status='ACTIVE') and not exists(select 1 from public.organizer_members om where om.organizer_id=(select organizer_id from public.competitions where id=v_group.competition_id) and om.user_id=v_uid and om.is_active=true and om.status='ACTIVE') then
   raise exception 'GROUP_EVENT_NOT_ELIGIBLE';
 end if;
 insert into public.chat_group_members(group_id,user_id,role,status) values(v_group.id,v_uid,'MEMBER','ACTIVE') on conflict do update set status='ACTIVE';
...
begin
 if not private.chat_group_user_is_admin(p_group_id,auth.uid()) then raise exception 'ACCESS_DENIED'; end if;
 update public.chat_groups set join_mode='link',invite_token_hash=encode(digest(v_token,'sha256'),'hex'),updated_at=now() where id=p_group_id;
 if not found then raise exception 'GROUP_NOT_FOUND'; end if;
 insert into public.chat_group_invites(group_id,token_hash,created_by) values(p_group_id,encode(digest(v_token,'sha256'),'hex'),auth.uid());
 return v_token;
end;$$;

create or replace function public.revoke_chat_group_invite(p_group_id uuid)
returns boolean language plpgsql security definer set search_path=public as $$
begin
 if not private.chat_group_user_is_admin(p_group_id,auth.uid()) then raise exception 'ACCESS_DENIED'; end if;
 update public.chat_groups set join_mode='invite_only',invite_token_hash=null,updated_at=now() where id=p_group_id;
 update public.chat_group_invites set revoked_at=now() where group_id=p_group_id and revoked_at is null;
 return true;
end;$$;

-- Collective participant variants. The token is validated against the existing competition session before membership/message access.
create or replace function public.list_collective_chat_groups(p_access_token text)
returns table(id uuid,name text,description text,group_type text,competition_id uuid,member_count bigint,last_message text,last_message_at timestamptz)
language sql security definer set search_path=public,private as $$
select g.id,g.name,g.description,g.group_type,g.competition_id,
 (select count(*) from public.chat_group_members m where m.group_id=g.id and m.status='ACTIVE'),
 (select m.body from public.chat_group_messages m where m.group_id=g.id order by m.created_at desc limit 1),
 (select m.created_at from public.chat_group_messages m where m.group_id=g.id order by m.created_at desc limit 1)
from public.chat_groups g join public.chat_group_members gm on gm.group_id=g.id
where gm.collective_participant_id=private.collective_group_identity(trim(coalesce(p_access_token,''))) and gm.status='ACTIVE'
order by coalesce((select max(m.created_at) from public.chat_group_messages m where m.group_id=g.id),g.updated_at) desc;
$$;

create or replace function public.get_collective_chat_group_messages(p_access_token text,p_group_id uuid,p_limit integer default 50,p_before timestamptz default null)
returns table(id uuid,sender_user_id uuid,sender_collective_participant_id uuid,sender_name text,sender_role text,body text,created_at timestamptz)
language sql security definer set search_path=public,private as $$
select m.id,m.sender_user_id,m.sender_collective_participant_id,
 coalesce(case when m.sender_collective_participant_id is not null then cp.full_name else p.full_name end,'Pengguna'),
 case when m.sender_collective_participant_id is not null then 'PESERTA' else 'GURU' end,m.body,m.created_at
from public.chat_group_messages m
left join public.profiles p on p.id=m.sender_user_id
left join public.collective_participants cp on cp.id=m.sender_collective_participant_id
where m.group_id=p_group_id and exists(select 1 from public.chat_group_members gm where gm.group_id=p_group_id and gm.collective_participant_id=private.collective_group_identity(trim(coalesce(p_access_token,''))) and gm.status='ACTIVE') and (p_before is null or m.created_at<p_before)
order by m.created_at desc limit greatest(1,least(coalesce(p_limit,50),100));
$$;

create or replace function public.send_collective_chat_group_message(p_access_token text,p_group_id uuid,p_body text)
returns public.chat_group_messages language plpgsql security definer set search_path=public,private as $$
declare v_cp uuid:=private.collective_group_identity(trim(coalesce(p_access_token,''))); v_row public.chat_group_messages; v_body text:=trim(coalesce(p_body,''));
begin
 if v_cp is null then raise exception 'COLLECTIVE_SESSION_INVALID'; end if;
 if char_length(v_body)<1 or char_length(v_body)>2000 then raise exception 'CHAT_BODY_INVALID'; end if;
 if not exists(select 1 from public.chat_group_members gm where gm.group_id=p_group_id and gm.collective_participant_id=v_cp and gm.status='ACTIVE') then raise exception 'ACCESS_DENIED'; end if;
 insert into public.chat_group_messages(group_id,sender_collective_participant_id,body) values(p_group_id,v_cp,v_body) returning * into v_row;
 update public.chat_groups set updated_at=now() where id=p_group_id;
 return v_row;
end;$$;
...
returns jsonb language plpgsql security definer set search_path=public,private as $$
declare v_cp uuid:=private.collective_group_identity(trim(coalesce(p_access_token,''))); v_group public.chat_groups; v_hash text:=encode(digest(trim(coalesce(p_invite_token,'')),'sha256'),'hex'); v_comp uuid;
begin
 if v_cp is null then raise exception 'COLLECTIVE_SESSION_INVALID'; end if;
 select * into v_group from public.chat_groups where invite_token_hash=v_hash and join_mode='link' limit 1;
 if not found then raise exception 'GROUP_INVITE_INVALID'; end if;
 select competition_id into v_comp from public.collective_participants where id=v_cp and status='ACTIVE';
 if v_group.group_type='event' and v_comp<>v_group.competition_id then raise exception 'GROUP_EVENT_NOT_ELIGIBLE'; end if;
 insert into public.chat_group_members(group_id,collective_participant_id,role,status) values(v_group.id,v_cp,'MEMBER','ACTIVE') on conflict do update set status='ACTIVE';
 return jsonb_build_object('ok',true,'group_id',v_group.id,'name',v_group.name);
end;$$;

-- Realtime: RLS only permits current full-account members. Collective participant realtime is added later through a scoped bridge,
-- while RPC reads/writes already enforce collective event membership.
do $$ begin
  if not exists(select 1 from pg_publication_tables where pubname='supabase_realtime' and schemaname='public' and tablename='chat_group_messages') then
    alter publication supabase_realtime add table public.chat_group_messages;
  end if;
end $$;

create policy chat_group_user_select on public.chat_group_messages for select to authenticated
using(private.chat_group_user_is_member(group_id,auth.uid()));
create policy chat_group_user_insert on public.chat_group_messages for insert to authenticated
with check(sender_user_id=auth.uid() and sender_collective_participant_id is null and private.chat_group_user_is_member(group_id,auth.uid()));

create or replace function private.chat_group_set_updated_at() returns trigger language plpgsql as $$ begin new.updated_at:=now(); return new; end; $$;
drop trigger if exists chat_group_touch on public.chat_groups;
create trigger chat_group_touch before update on public.chat_groups for each row execute function private.chat_group_set_updated_at();
